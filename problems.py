...
class KnapsackProblem(Problem):
    ...
    Example:
        https://github.com/moead-framework/framework/blob/master/moead_framework/problem/combinatorial/knapsack.py
    """
...
class MaximumCliqueProblem(Problem):
    """
    Implementation of the Multiobjective maximun clique problem by Luis Alfredo Avendaño Muñoz.
    The problem is compatible with files available on the
    author website: https://networkrepository.com/
    Example:
    ...
    def __init__(self, number_of_objective, instance_file, problem_type="MWC"):
        """
        Constructor of the problem
        :param number_of_objective: {integer}
        :param instance_file: {string} txt file of the instance: https://networkrepository.com/
        """
        super().__init__(number_of_objective)
    ...
    def f(self, function_id, decision_vector):
        ...
        function_id = function_id - 1
        subgraph_list = np.where(np.array(decision_vector) == 1)[0]
        ...
        if self.problem_type == "BOMWC":
            ...
            raise AttributeError(f"Problem type {self.problem_type} not recognized")
